...
class HouseFeatures(BaseModel):
    ...
    AveOccup: float = Field(
        gt=0,
        description="Average occupancy per household"
    )
...
def predict(house:HouseFeatures):
  try:
    ...
    "confidence_range": f"${price_usd-3900:,.0f} to ${price_usd+3900:,.0f}"
}
...
async def predict_file(file:UploadFile=File(...)):
  ...
  if missing_columns:
    raise HTTPException(
      status_code=400,
      detail=f"these columns missing from your file {missing_columns}"
    )
  if len(df)==0:
    raise HTTPException(
      status_code=400,
